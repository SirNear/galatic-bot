...
module.exports = class apteste extends Command {
  ...
  async run({ message, args, client, server }) {
    ...
    async function buscarNaPlanilha(spreadsheetId, range, target) {
      try {
          ...
          for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
              ...
              const col1Norm = normalizeText(col1);

              if (col1Norm === target || (col1Norm.length >= 3 && col1Norm.includes(target)) || (target.length >= 3 && target.includes(col1Norm))) {
                  resultados.push({
                      aparencia: col1,
                      universo: col2,
                      personagem: col3,
  ...
  async function sistemaRegistroPaginacao(msgNavegacao, ii, target, resultados, options) {
    const exactMatchFound = resultados.some(r => normalizeText(r.aparencia) === target);
    let EmbedPagesAparencia = resultados.map((r, idx) =>
        new EmbedBuilder()
    ...
    if (!exactMatchFound) {
        const embedRegistro = new EmbedBuilder()
            ...
            .setDescription(`Aparência **${target}** disponível. Também encontramos resultados similares.\nSelecione "sim" para registrar e "não" para ver os outros resultados disponíveis.`)
            .setColor("#00ff00");
    ...
    resultados.forEach((r, idx) => {
        ...
        EmbedPagesAparencia[targetIndex]
            ...
            .setFooter({
                text: `Página ${targetIndex + 1}/${EmbedPagesAparencia.length} - ⏩ = proxima página | ⏪ = página anterior | ❌ = cancelar busca`,
            });
    });
    ...
    const navCollector = msgNavegacao.createMessageComponentCollector({
        filter: (ii) => ii.user.id === options.message.author.id,
        time: 60000,
    });
    ...
    navCollector.on("collect", async (ii) => {
        switch (ii.customId) {
            case 'sim_ap':
                ...
                const campoNome = new TextInputBuilder().setCustomId("campoNome").setLabel("Nome da Aparência").setStyle(TextInputStyle.Short).setRequired(true);
            ...
            case 'close_ap':
                ...
                options.message.reply({ content: "<a:cdfpatpat:1407135944456536186> | **NAVEGAÇÃO FINALIZADA!**" }).catch(() => {});
                break;
        }
    });
...
async function sistemaRegistroAvulso(msgNavegacao, target, options) {
    ...
    coletorBotoesRegistro.on("collect", async (ii) => {
        switch (ii.customId) {
            ...
            case 'nao_ap':
                await msgNavegacao.delete().catch(() => {});
                await options.message.reply({ content: "<a:cdfpatpat:1407135944456536186> | **REGISTRO CANCELADO!** Tudo bem, você pode reservar depois, se estiver disponível" }).catch(() => {});
                break;
        }
    });

    coletorBotoesRegistro.on("end", async (collected, reason) => {
        if (reason === "time" && collected.size === 0) {
            await msgNavegacao.edit({ embeds: [], components: [] }).catch(() => {});
            options.message.reply({ content: "Tempo esgotado." }).catch(() => {});
        }
    });
}
    ...
    switch (args[0]) {
      case 'registrar':
        sistemaRegistroAvulso(msgNavegacao, null, nomeAparencia, resultados, { message, client, sChannel })
      break;
      default:
        ...
        const embedNavegacao = new EmbedBuilder()
          ...
          .setDescription(
            "Escolha o que você deseja conferir a disponibilidade"
          )
          ...
          switch (i.customId) {
            case "botaoNavAparencia":
              ...
              const embedAparencia = new EmbedBuilder()
                ...
                .setDescription(
                  "Envie no chat a aparência que deseja verificar."
                )
                .setFooter({
              ...
              coletorAparencia.on("collect", async (m) => {
                  ...
                  case 1:
                    ...
                    EmbedPagesAparencia[0]
                      ...
                      .addFields(
                        {
                          name: "**APARÊNCIA**",
                          value: found.aparencia ?? "—",
                        },
                        { name: "**UNIVERSO**", value: found.universo ?? "—" },
                        {
                          name: "**PERSONAGEM**",
                          value: found.personagem ?? "—",
                        },
                ...
                if (resultados.length >= 1) {
                  const exactMatchFound = resultados.some(
                  ...
                  resultados.forEach((r, idx) => {
                    ...
                    EmbedPagesAparencia[targetIndex]
                      .setDescription(
                        `<:patrickconcern:1407564230256758855> | Resultado ${
                          idx + 1
                        }`
                      )
                      .addFields(
                      ...
                      case "sim_ap":
                        ...
                        const filter = (modalInteraction) =>
                          modalInteraction.customId ===
                            "esqueletoFormularioRegistro" &&
                          modalInteraction.user.id === ii.user.id;
            ...
            case "verso":
              ...
              /* #region contador */
              intervalo,
                (contador = await iniciarContador(
                  tempoRestante,
                  sujeito,
                  msgAlvo,
                  message
