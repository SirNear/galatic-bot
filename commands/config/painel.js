...
module.exports = class painel extends Command {	
	...
	constructor(client) {
     ...
     switch(args[0]) {
        case "ver": {
          // Embed principal com as configurações do servidor
          ...
          const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('1').setLabel('CANAL DE PUNIÇÕES').setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId('2').setLabel('PREFIX').setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId('3').setLabel('CARGO DE MUTE').setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId('4').setLabel('SISTEMA DE MUTE AUTOMATICO').setStyle(ButtonStyle.Primary)
          );

          const row12 = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('5').setLabel('CARGO DE MODERAÇÃO').setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId('6').setLabel('CATEGORIA DE MONITORAMENTOS').setStyle(ButtonStyle.Primary)
          );

          // Envia o embed com os botões e cria coletor
          message.channel.send({ embeds: [embedv], components: [row, row12] }).then((msg) => {
            ...
            collector.on("collect", async (i) => {
              switch (i.customId) {
                case "1": {
                  // Alterar canal de punições
                  let msgPuni = await message.channel.send({ content: '***Mencione o novo canal de punições***' });
                  const collectorMsg = await msgPuni.channel.createMessageCollector({
                ...
                case "2": {
                  ...
                  const collector2 = await msgPrefix.channel.createMessageCollector({
                    filter: (m) => m.author.id === message.author.id,
                    time: 120000,
                    max: 1
                  });
                ...
                case "3": {
                  // Alterar cargo de mute
                  let msgMutero = await message.channel.send({ content: '**Mencione o novo cargo de mute**' });
                  const collector3 = await msgMutero.channel.createMessageCollector({
                    filter: (m) => m.author.id === message.author.id,
                    time: 120000,
                    max: 1
                  });
                  ...
                  collector3.on('collect', (collected) => {
                    ...
                    server.save();
                    message.channel.send(`**Cargo de mute alterado para ${novoCargo}`); // mantenho a string original
                  });
                  break;
                }

                case "4": {
                  // Configuração do sistema de auto silenciamento (embed de ajuda + ações)
                  ...
                  if (server.warnTag.includes === 'Desativado') {
                    // Se estiver desativado: opção para ativar
                    helpAutoMute.setDescription(
                      `O sistema de Auto Silenciamento acontece aplicando um mute automático após um número x de warns/avisos, sendo configuravél para ativar ou desativar e também o número de avisos necessários. \n \n <:dnd:572210462993940482> | Status Desativado. \n \n Reaja com "<:StatusOn:572210002039668818>" para **Ativar** o sistema.`
                    );

                    const row2 = new ActionRowBuilder().addComponents(
                      new ButtonBuilder().setCustomId("41").setLabel("<:StatusOn:572210002039668818>").setStyle(ButtonStyle.Primary)
                    );
                      ...
                      collectoram.on("collect", async (i) => {
                        ...
                        message.channel.send(`**O sistema de auto Silenciamento foi ativado com o máximo de \`${server.warnNumber} warns\`. Reaja com "<a:a:moderacao:569064320156172299>" para alterar este valor.`).then(ms => {
                          const row3 = new ActionRowBuilder().addComponents(
                            new ButtonBuilder().setCustomId("411").setLabel("<a:a:moderacao:569064320156172299>").setStyle(ButtonStyle.Primary)
                          );
                    ...
                    // Se estiver ativado: opções para desativar ou editar número de warns
                    helpAutoMute.setDescription(
                    ...
                    const row4 = new ActionRowBuilder().addComponents(
                      new ButtonBuilder().setCustomId("421").setLabel("<a:negativo:563096795907883010>").setStyle(ButtonStyle.Primary),
                      new ButtonBuilder().setCustomId("422").setLabel("<a:moderacao:569064320156172299>").setStyle(ButtonStyle.Primary)
                    );
                        ...
                        switch (i.customId) {
                          ...
                          case "422":
                            message.channel.send('**Envie o novo número de warns máximos(Apenas números)**').then(async (msg422) => {
                              const coletorWN2 = await msg422.channel.createMessageCollector({
                                filter: (m) => m.author.id === message.author.id,
                                time: 120000,
                                max: 1
                              }).then(collected => {
                ...
                case "5": {
                  ...
                  const row5 = new ActionRowBuilder().addComponents(
                    // Corrigido typo para setStyle (mesma intenção)
                    new ButtonBuilder().setCustomId("51").setLabel("<:lolipolice:669705464447107073>").setStyle(ButtonStyle.Primary)
                  );
                  ...
                  message.channel.send({ embeds: [embedInit], components: [row5] }).then((m) => {
                    ...
                    collectorcm.on("collect", async (i) => {
                      ...
                      message.channel.send({ content: '**Mencione o novo cargo de Moderação**' }).then(async (msg2) => {
                        const coletorstaffRole = await msg2.channel.createMessageCollector({
                          filter: (m) => m.author.id === message.author.id,
                          time: 120000,
                          max: 1
                        }).then(collected => {
                ...
                case "6": {
                  // Alterar categoria de monitoramento
                  ...
                  const row6 = new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId("61").setLabel("ALTERAR").setStyle(ButtonStyle.Primary)
                  );
                  ...
                  collectormc.on("collect", async (i) => {
                    ...
                    let prompt = await message.channel.send({
                      content: '**Digite o nome ou ID da nova categoria. Inclua espaçamentos, acentos, pontuações e outros caracteres especiais. \n \n AVISO: Pode demorar até 15s para confirmação**'
                    });

                    const coletornewCategory = await prompt.channel.createMessageCollector({
                      filter: (m) => m.author.id === message.author.id,
                      time: 120000,
                      max: 1
                    });
                    ...
                    coletornewCategory.on('collect', async (collected) => {
                      ...
                      if (!newCategory) {
                        ...
                        message.channel.send({
                          content: `**Categoria de Monitoramento alterada para ${message.guild.channels.cache.find(c => c.id == server.monitorCategory && c.type == "category").name}**`
                        });
                      }
                    });
                  });
                ...
                default: {
                  // Caso sem argumento válido: mostra help rápido
        ...
        default: {
          // Sem argumento: mostra instruções rápidas
          const embedd = new EmbedBuilder()
            .setTitle('<:engrenagem:564590880707837991> | ***Painel do Servidor***')
            .setDescription(`<:dnd:572210462993940482> | Utilize \`${server.prefix}painel ver\` para editar o painel.`)
            .addFields({
